...
function display_menu() {
  clear
  echo "This is a standalone Privtae Service Edge  Troubleshooting Script"
  ...
  echo "-----------------------------------------------------------------------------------------------"
  echo
  echo "1. Show Privtae Service Edge  status"
  echo "2. Reprovision Privtae Service Edge "
...
function task1() {
  ...
  systemctl status zpa-service-edge -l
  echo
}
...
FILE="/opt/zscaler/var/service-edge"
provision_key="NULL"
confirm="no"
yes="yes"
...
echo "********************************************************"
echo "*                                                      *"
echo "*       Re-Provision ZPA Private Service Edge.         *"
echo "*                                                      *"
echo "********************************************************"
echo 
...
sleep 2
clear
sudo watch -n 1 systemctl status zpa-service-edge
else
    echo "Cancelling..."
    exit
fi 
...
function task3() {
    ...
    sudo journalctl -u zpa-service-edge ${since_param:+--since="$since_param"} > "$output_file"
}
...
function task6() {
  ...
  journalctl -u zpa-service-edge > "$output_file2"
  echo    
